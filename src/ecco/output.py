...
class OutputSeq:
    ...
    def _get_encoder_hidden_states(self):
        return (
            ...
            else self.decoder_hidden_states
        )
    ...
    def __str__(self):
        return "<LMOutput '{}' # of lm outputs: {}>".format(
            self.output_text, len(self._get_hidden_states()[1][-1])
        )
    ...
    def explorable(self, printJson: Optional[bool] = False):
        ...
        for idx, token in enumerate(self.tokens[0]):
            ...
            tokens.append(
                {"token": token, "token_id": int(self.token_ids[0][idx]), "type": type}
            )
    ...
    def position(self, position, attr_method="grad_x_input"):
        if (position < self.n_input_tokens) or (position > len(self.tokens) - 1):
            raise ValueError(
                ...
                "Accepted values for this sequence are between {} and {}.".format(
                    self.n_input_tokens, len(self.tokens) - 1
                )
            )
        ...
        assert (
            attr_method in self.attribution
    ...
    def layer_predictions(
        ...
        **kwargs,
        ...
        assert (
            dec_hidden_states is not None
        ), "decoder hidden states not found, please use output_hidden_states=True"
        ...
        if self.model_type in ["enc-dec", "causal"]:
            ...
            assert new_position >= 0, (
                f"position={position} not supported, minimum is "
                f"position={self.n_input_tokens + offset} for the first generated token"
            )
            assert new_position < len(dec_hidden_states), (
                f"position={position} not supported, maximum is "
        ...
        dec_hidden_states = dec_hidden_states[position][
            :, -1, :
        ]  # only focus on the hidden states for that particular position
    ...
    def rankings(self, **kwargs):
        ...
        lm_plots.plot_inner_token_rankings(
            input_tokens, output_tokens, rankings, **kwargs
        )
    ...
    def attention(self, attention_values=None, layer=0, **kwargs):
        ...
        importance_id = (
            self.n_input_tokens - 1
        )  # Sete first values to first output token
        tokens = []
        if attention_values:
...
class NMF:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        merged_act = self.reshape_activations(
            activations, from_layer, to_layer, collect_activations_layer_nums
        )
        ...
        self.model = decomposition.NMF(
            n_components=n_components, init="random", random_state=0, max_iter=500
        )
    ...
    def reshape_activations(
        ...
        if len(activations.shape) != 4:
            raise ValueError(
                ...
                f"Supplied dimensions: {activations.shape}",
                "activations",
            )
    ...
    def explore(self, input_sequence: int = 0, **kwargs):
        ...
        if len(self.token_ids[input_sequence]) != self.n_input_tokens:
            ...
            factors = [
                comp.tolist() for comp in factors
            ]  # the json conversion needs this
        else:
            # Case: no generation
            factors = [
                comp.tolist() for comp in self.components
            ]  # the json conversion needs this
